...
| **Effects** | ❌ 持續執行，不會被清理 | ✅ 隱藏時清理，可見時重新同步 |
| **重新渲染** | ⚠️ 隱藏時仍會處理更新 | ✅ 隱藏時仍會處理更新，但**優先級較低** |
...
- 需要還原有(休眠/暫停)的狀態<sup>註1</sup>
- 需要預先載入資料或渲染UI，並在客戶端元件中進行顯示或隱藏(例如: 搜尋結果、資料預先載入)

> 註1: 這裡的狀態是混合了React中的狀態和DOM狀態，例如: 捲軸位置、表單輸入或`<video>`,`<audio>`,`<iframe>`等
